...
final class SimpleAccessibleRegistry<T> implements AccessibleRegistry<T> {

    public static <T> AccessibleRegistry<T> of(T defaultInstance) {
        return new SimpleAccessibleRegistry<>(defaultInstance);
    }
    ...
    SimpleAccessibleRegistry(T defaultInstance) {
        this.registered = new ConcurrentHashMap<>();
        this.defaultInstance = Objects.requireNonNull(defaultInstance);
    }

    @Override public AccessibleRegistry<T> register(T rateLimiterFactory) {
        defaultInstance = Objects.requireNonNull(rateLimiterFactory);
        return this;
    }

    @Override public AccessibleRegistry<T> register(String name, T rateLimiterFactory) {
